def get_model(model_name, args):
    ...
    elif name == "finetune_wa":
        from models.finetune_wa import Finetune
        return Finetune(args)
    elif name == "replay":
        from models.replay import Replay
        return Replay(args)
    elif name == "replay_wa":
        from models.replay_wa import Replay
        return Replay(args)
    elif name == 'replay_init':
        from models.replay_init import Replay
        return Replay(args)
    elif name in ("wsc", "replay_ours_swa"):
        # Weight Space Consolidation (paper): `replay_ours_swa` was the in-code name; `wsc` is canonical.
        from models.wsc import WSC

        return WSC(args)
    elif name == "replay_ours":
        raise ValueError(
            "model_name 'replay_ours' is not Weight Space Consolidation. "
            "The paper method is implemented as `replay_ours_swa` (legacy string) or `wsc` (preferred); "
            "see models/wsc.py."
        )
    elif name == "gem":
